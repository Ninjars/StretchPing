...
class PlannerEventToState(private val idProvider: IdProvider) : (State, PlannerUIEvent) -> State {
    override fun invoke(state: State, event: PlannerUIEvent): State = when (event) {
        ...
        is PlannerUIEvent.UpdateSectionEntryTransitionDuration ->
            state.copy(
                sections = state.sections.update(event.id) {
    ...
    private fun List<Section>.update(id: String, func: (Section) -> Section) = map {
        if (it.id == id) {
            func(it)
        } else {
            it
    ...
    private fun List<Section>.reorder(fromIndex: Int, toIndex: Int): List<Section> = toMutableList().apply {
