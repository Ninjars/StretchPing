...
object SettingsStateToViewState : (SettingsVM.State) -> SettingsViewState {
    private const val VERSION_CODE: String = BuildConfig.VERSION_CODE.toString()
    private const val VERSION_NAME: String = BuildConfig.VERSION_NAME

    override fun invoke(state: SettingsVM.State): SettingsViewState = SettingsViewState(
        ...
        versionInfo = "- $VERSION_NAME ($VERSION_CODE) -",
    )
}

private fun NavLabelDisplayMode.toStringResId() = when (this) {
...
private fun ThemeMode.toStringResId() = when (this) {
    ThemeMode.Unset -> -1
