...
internal object ActiveTimerStateUpdater : (State, Command?) -> State {
    ...
    override fun invoke(state: State, command: Command?): State = when (command) {
        null -> state
    ...
    private fun startSegment(state: State, command: Command.StartSegment): State = state.copy(
        hasStarted = true,
        activeSegment = state.segments[command.index].toActiveSegment(command.startMillis),
        index = command.index,
    )

    private fun restart(state: State, command: Command.RepeatExercise): State = state.copy(
        hasStarted = true,
    ...
    private fun State.SegmentSpec.toActiveSegment(startMillis: Long): State.ActiveSegment = State.ActiveSegment(
    ...
    private fun pauseActiveSegment(state: State, command: Command.PauseSegment): State = state.copy(
        hasStarted = true,
